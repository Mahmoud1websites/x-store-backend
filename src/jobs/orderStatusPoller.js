...
async function pollOnce() {
  ...
  const supplierLabel = order.supplier_source || 'kamal_cell';   // ← new line
  try {
    const updated = await orderService.recheckOrder(order.order_uuid);

    if (updated.status === 'wait') {
      console.log(
        `[orderStatusPoller] (${supplierLabel}) order ${order.order_uuid} is still waiting`   // ← added (${supplierLabel})
      );
    } else {
      console.log(
        `[orderStatusPoller] (${supplierLabel}) order ${order.order_uuid} resolved -> ${updated.status}`   // ← added (${supplierLabel})
      );
    }
  } catch (error) {
    console.error(
      `[orderStatusPoller] (${supplierLabel}) failed to recheck ${order.order_uuid}:`,   // ← added (${supplierLabel})
      error.message
    );
  }
}
